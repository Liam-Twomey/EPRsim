...
def spline_interpolation_angle_grid(Par, intensity, res):
    ...
    if hasattr(Par, "oritheta"):
        ...
        intensity = make_orientationdepency_theta(
            intensity, theta, pos_theta, sigma_theta
        )
...
def make_orientationdepency_theta(intensity, theta, pos_theta, sigma_theta):
    ...
    for k in range(0, Par._ntheta):
        intensity[:, k, :] = intensity[:, k, :] * (
            np.exp(-0.5 * (theta[k] - pos_theta) ** 2 / sigma_theta ** 2)
            + np.exp(
                -0.5
                * (theta[Par._ntheta - k - 1] - pos_theta - np.pi / 2) ** 2
                / sigma_theta ** 2
            )
            + np.exp(
                -0.5
...
def make_orientationdepency_phi(Par, intensity, phi, pos_phi, sigma_phi):
    ...
    for k in range(0, Par._nphi):
        intensity[:, :, k] = intensity[:, :, k] * (
            np.exp(-0.5 * (phi[k] - pos_phi) ** 2 / sigma_phi ** 2)
            + np.exp(-0.5 * (phi[k] - pos_phi + 2 * maxphi) ** 2 / sigma_phi ** 2)
            + np.exp(-0.5 * (phi[k] - pos_phi - 2 * maxphi) ** 2 / sigma_phi ** 2)
        )
    return intensity
...
def spline_interpolation_analytical_resInt(
    ...
    for tr in range(0, ntransitions):
        for i in range(0, Par._ntheta):
            ...
            intensity2[tr, i, 0 : i + 1] = interpolate.splev(
                phi_proj[i, 0 : i + 1], spl
            )
            spl2 = interpolate.splrep(phi, resonance[tr, i, :], k=1)
            resonance2[tr, i, 0 : i + 1] = interpolate.splev(
                phi_proj[i, 0 : i + 1], spl2
            )
    # rename it for output
