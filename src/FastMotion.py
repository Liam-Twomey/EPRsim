...
def fast_motion_kernel(Param, SimPar):
	...
	Parameters
	----------
	Sys: eprsim.EPRsim.Parameters
		Definitions of spin system parameters
	Exp: eprsim.EPRsim.Parameters?
	...
	Int
		Intensity of one cw-EPR spectrum
	Harmonic: bool
		Boolean variable. 1 = first derivative. 0 = absorptive

	The calculation is basically the same for isototropic and fast-motion
	spectra except of the linewidth. Further differences between fast-motion
	and isotropic calculations were set up before calling this function
	(expansion of hyperfine vectors in the redefine_nuclear_coupling() function).
...
def create_fastmotion_spectrum(Bfield,resonances,indices,I,Deltag,DeltaA,tcorr,	lw,	lwG,
	giso,mfreq,	Harmonic=1):
	"""
	...
	resonances
		vector with resonance indices in Bfield
	indices
		list with all index vectors of the coupled system
	I
		vector with nuclear spin quantum number for all nuclei
	Deltag
		g-giso*1 tensor
	DeltaA
		A-Aiso*1 tensor
	tcorr
		rotational correlation time in s
	lw
		Lorentzian linewidth given in mT as FWHM
	lwG
		Gaussian linewwidth given in mT as FWHM
	giso
		Isotropic g-value of the system
	mfreq
		Experimental microwave fequency in Hz
	Harmonic: int{1,2}
		Sets harmonic to simulate(1 = first derivative. 2 = absorptive)

	Returns
	-------
	Int
		Intensity vector of the spectrum
...
def create_isotropic_spectrum(Bfield, resonances, indices, equiv, I, Harmonic, lw, lwG):
	...
	indices
		list with all index vectors of the coupled system
	equiv
		vector with number of equivalent nuclei
	I
		Vector with nuclear spin quantum number for all nuclei
	Harmonic:int{1,2}
		Sets harmonic to simulate (1 = first derivative. 2 = absorptive)
...
def calculate_resfields(A, g, Nucs, equiv, mfreq, Iequiv, g_n):
	...
	----------
	A
		vector of hyperfine tensors given as 3x3 matrix
	g
		g-tensor as 3x3 matrix
	Nucs
		string with coupling nuclei
	equiv
		vector with numbers of equivalent coupling nuclei
	mfreq
		experimental microwave frequency in Hz

	Returns
	-------
	resfields:list
		list of resonance fields
	indices_list: list
		list of indices of mI quantum numbers
	DeltaA_list: list
		list of A-Aiso*1 tensors
	Deltag_list
		list of g-giso*1 tensors
	I_list
		list of nuclear spin quantum numbers
	giso
		isotropic g-value
...
def Breit_Rabi_iteration(aiso, giso, I, g_n, mfreq, k=0):
	...
	coupled to exactly one nucleus.
	...
	g_n
		nuclear g-value for the used isotope
	mfreq
		experimental microwave frequency in Hz
	k: int
		k = 0:first nucleus, k > 0 all other splittings
	
	Returns
	-------
	resfields
		list of resonance fields for the given input
	indices
		list of mI quantum numbers

	If k > 0 the Biso resonance position is subtracted from the resonance fields.
	This resonance splittings are subsequently introduced to the k = 0 resonance
	fields via first order corrections. The function uses the Breit-Rabi formula
	for the resonance condition and solves the implicit function via fixpoint iteration.
...
def Biso(giso:float, mfreq:float, unit:str="mT"):
	"""
	...
	unit: str{'mT','T','G'}
		unit in which the magnetic field value is returned

	Returns
	-------
	B_iso: float
		magnetic field value for the given giso
...
def do_hf_splitting(indices_list, resfield_list):
	...
	resfield vector (splitting with k = 0, see Breit_Rabi_iteration()).

	Parameters
	----------
	indices_list: list
		initial list with mI values
	resfield_list: list
		initial list of not coupled resonance fields)
	
	Returns
	-------
	resfields: list
		list of coupled resonance fields
	indices_list: list
		multidimensional list of mI values

	Notes
	-----
	The function returns the resonance fields for all coupled hyperfine
...
def tensor_readout(A, g):
	...
	----------
	A
		hyperfine tensor, given as 3x3 matrix)
	g
		g-tensor, given as 3x3 matrix)
...
def fast_motion_lw(Deltag, DeltaA, I, mI, Biso, giso, tcorr, Nresonances):
	...
	by the fast_motion_lw_kernel() function.
	...
	Deltag_list
		g-giso*1 tensor
	DeltaA
		vector with with A-Aiso*1 tensors
	I
		list with nuclear spin quantum numbers
	mI
		list with mI projection quantum numbers
	Biso
		central magnetic field
	giso
		isotropic g-value
	tcorr
		rotational correlation time, given in s
	Nresonances
	...
	-------
	lw_s
		vector with anisotropic linewidths in mT as FWHM
...
def fast_motion_lw_kernel_expired(
	...
	Deltag
		g-Aiso*1 tensor
	a11
		scalar factor for Kivelson formula
	a22
		scalar factor for Kivelson formula
	b11
		scalar factor for Kivelson formula
	...
	field_dep
		scalar factor for Kivelson formula
	mI
		vector with mI projection quantum numbers
	I
		vector with nuclear spin quantum numbers
	Nresonances
		number of resonances
	...
	-------
	lw_s
		vector with anisotropic linewidths given in Hz and FWHM

	Notes
	-----
	The anistropic linewidth formula is according to N.M.Atherton (p.329-330).
	It should ne noted that the pseudo-secular contributions are not neglected.
...
def fast_motion_lw_kernel(
	...
	The anistropic linewidth formula is according to N.M.Atherton (p.329-330). It
	should ne noted that the pseudo-secular contributions are not neglected.
	Just-in-time complilation of this function is used if Numba is available.
...
def create_Lorentzian(x, x0, lw, Harmonic=1):
	"""
	Generates a Lorentzian signal from passed parameters	

	Parameters
	----------
	x
		x-vector, here the magnetic field vector)
	x0
		x0, here the resonance field)
	lw
		Lorentzian linewidth, given in mT as FWHM)
	Harmonic
		1 = first derivative. 2 = absorptive)
	
	Returns
	-------
	L
		spectrum with Lorentzian lineshape)
...
def field_interpolation(Int, Points, Bfield):
	...
	----------
	Int
		intestity vector of the input signal
	Points
		whished number of points of the output signal
	Bfield
		magnetic field vector of the input signal
	
	Returns
	-------
	field
		interpolated field vector with Points elements
	signal
		interpolated intensity vector with Points elements
...
def do_default_range(resfield, lw, lwG, tcorr=None):
	...
	resfield
		resonance fields
	lw
		linewidth parameters Lorentzian
	lwG
		linewidth parameter Gaussian
	Iso
		boolean variable. If true than it is a isotropic calc.
	tcorr
		rotational correlation time. None for isotropic calc.

	Returns
	-------
	Bfield: np.ndarray
		Magnetic field vector (linspace)
...
def print_Info(SimPar):
	"""
	Prints information about the simulation pass, equivalent to __str__	
