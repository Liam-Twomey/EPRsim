...
def simulate(Parameters):
	...
	(isotropic, fast-motion and solid state). All spectra are simulated as field sweep spectra.

	Isotropic/fast-motion
		For the fast-motion regime/isotropic limit, the program solves
		the implicit Breit-Rabi formula [1]_ in a fixed-point iteration.
		Anisotropic line-broadening effects in the fast-motion regime
		are calculated via the Kivelson formula [2]_ .
	...
	Solid-state
		...
		of eigenvalues and transition probabilitites (similar to [1]_ ).
	...
	1. Matrix is too large for solid-state simulation.
	2. Fast-motion simulation is not possible due to S > 1/2.
	...
	>>> P.A = [12, 13, 110]
	>>> P.Nucs = '14N'
	>>> P.lw = [0.2, 0.2]ImportError: attempted relative import with no known parent package
	>>> P.tcorr = 3e-9
	>>> P.motion = 'fast'
	>>> B0, spc, flag = sim.simulate(P)
	>>> tool.plot(B0, spc)
...
def redefine_nuclear_coupling(Param):
	...
	#. Get the hyperfines and Nucs definition from the Sys class
	#. Check if equivalent nuclei are defined in the fast motion regime
	#. If 2 is true: Get an expansion with all combination of isotopes
	#. Check if equivalent nuclei are defined with nonpure isotopes
	#. If 4 is True: Do the same as in 2 and 3 for fast motion.
	#. Remove all isotopes mixture below a given probability threshold
	#. Remove uncessary permutation of isotopes within a equivalent group
	
	Warnings
	--------
	* Works directly on the Sys class member variables due to call by
	  reference!
...
def check_eq_in_fast_motion(Param, Force=False):
	...
	Notes
	-----
	Equivalent nuclei has to be treated separately in the fast motion regime.
	Therefore if an input is given as :code:`Sys.n = [2,2], Sys.Nucs = "N,H";
	...
	Warnings
	--------
	Works directly on the passed variables due to call by reference.
...
def create_nucvec(Ind, NucIso):
	...
	NucIso:
		vector of isotopes for the nuclei
...
def remove_permutations(Par, _Nucsvec, w, Force=False):
	...
	Notes
	-----
	Assume a spin system with n = [2,2], Nucs = "N,H" is transformed in
	the fast-motion regime to a system "N,N,H,H". There requirement is
	still that the coupling is to equivalent nuclei (in general the A-tensor is
	identical within the H's and N's). The isotopes combiniations above the
	probability threshhold will be
	...
	Since 2. and 3. (and 4 and 5) are indentical (just a perumation of an isotope
	within the equivalent coupling nuclei group) one of them can be
...
class Parameters(object):
	...
	Solver : :class:`string`, 
		...
		**Only relevant in slow-motion**. The Lanczos algorithm is used
		as a default to solve the stochastic Liouville equation.
	
	LMKmax : :class:`list` of :class:`int`
		Maxmimum rank of L, M and K for the spatial basis.
		**Only relevant in slow-motion**. The slow-motion algorithm takes
		a default basis if LMKmax is not provided.
	
	SNR : :class:`float`
		Signal-to-noise ratio for addition of artificial white noise.
